...
class S4STransactionPayment(Document):
	def on_submit(self):
		self.update_pay_entry_details()

	def before_save(self):
		self.update_pay_entry_details()

	def update_pay_entry_details(self):
		for row in self.approval_details:
			if row.payment_entry:
				pe = frappe.get_doc("Payment Entry",row.payment_entry)
				if pe.docstatus == 0:
					if row.chequeref_no:
						pe.reference_no = row.chequeref_no
					if row.chequeref_date:
						pe.reference_date = row.chequeref_date
	...
	def get_data(self):
		...
		for dataset in query:
			if not frappe.db.get_value("S4S Transaction Payment Item",{"transaction_no":dataset.get("s4s_p_name"),"docstatus":["in",[0,1]],"parent":["!=",self.name]},["parent"]):

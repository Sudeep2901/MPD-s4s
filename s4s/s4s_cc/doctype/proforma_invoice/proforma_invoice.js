...
frappe.provide("s4s.s4s_cc");


s4s.s4s_cc.ProformaInvoiceController = erpnext.selling.SellingController.extend({
	setup: function(doc) {
		...
		this.$sales_order_btn = this.frm.add_custom_button(__('Sales Order'),
			function() {
				erpnext.utils.map_current_doc({
					method: "s4s.s4s_cc.doctype.proforma_invoice.proforma_invoice.make_sales_invoice_from_so",
					...
					get_query_filters: {
						...
						proforma_invoice: ["in",["",null,undefined]]
					}
				})
			}, __("Get Items From"));
	},
	...
	delivery_note_btn: function() {
		...
		this.$delivery_note_btn = this.frm.add_custom_button(__('Delivery Note'),
			function() {
				erpnext.utils.map_current_doc({
					method: "s4s.s4s_cc.doctype.proforma_invoice.proforma_invoice.make_sales_invoice_from_dn",
...
$.extend(cur_frm.cscript, new s4s.s4s_cc.ProformaInvoiceController({frm: cur_frm}));
...
frappe.ui.form.on('Proforma Invoice', {
	setup: function(frm){
		...
		frm.set_query('cc_name', function(doc) {
			return {
				filters: {
					company: frm.doc.company
				}
		...
		cur_frm.remove_custom_button("Fetch Timesheets")
		cur_frm.remove_custom_button("Quotation","Get Items From")
	},
	...
	company: function(frm){
		...
		if (frm.doc.company){
			frappe.call({
				method:"fetch_address",
				doc:frm.doc,
				args:{company:frm.doc.company},
				callback:function(r){
					if(r.message){
						frm.set_value("company_address",r.message)
						frm.set_value("dispatch_address_name",r.message)
					}
				...
				frappe.call({
					...
					callback: function(r){
						if(r.message){
							...
							if (frm.doc.delivery_note_ms.length == 0){
								frm.set_value("delivery_note_ms",data)
...
var add_to_item_line = function(frm, checked_values, invoice_healthcare_services){
	if(invoice_healthcare_services){
		...
		for(let i=0; i<checked_values.length; i++){
			var si_item = frappe.model.add_child(frm.doc, 'Proforma Invoice Item', 'items');

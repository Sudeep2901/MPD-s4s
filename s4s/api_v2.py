...
def otp_verification(mobile_no,token,otp):
    ...
    if frappe.db.exists("OTP Users Data",{"parent":"S4S OTP Verification","mobile_number":mob_no or "+91"+mob_no}):
        ...
        if frappe.db.get_value("OTP Users Data",{"parent":"S4S OTP Verification","mobile_number":mob_no or "+91"+mob_no},"timestamp")==timestamp and frappe.db.get_value("OTP Users Data",{"parent":"S4S OTP Verification","mobile_number":mob_no or "+91"+mob_no},"otp")==otp:
            ...
            if frappe.db.exists("User Permission",{"user":user_doc.name,"allow":"Company"}):
                up = frappe.get_doc("User Permission",{"user":user_doc.name,"allow":"Company"})
                company = up.for_value
...
def get_farmer_list(user_id,filters=None,page_no=1,kyc_status=None):
    ...
    final_condition = ""
    cc_list,vlcc_list=[],[]
    if kyc_status == "0" or kyc_status == 0:
        all = frappe.db.sql("""
            SELECT 
        ...
        """.format("AND "+conditions if conditions!="" else conditions),values,as_dict=1)
    elif kyc_status == "1" or kyc_status == 1:
        all = frappe.db.sql("""
            SELECT 
            ...
            order by ss.creation desc
        """.format(" AND "+conditions if conditions!="" else conditions),values,as_dict=1)
    elif kyc_status == "2" or kyc_status == 2:
        all = frappe.db.sql("""
            SELECT 
    ...
    result = [
        i for i in all if frappe.has_permission("S4S Supplier", user=user_id, doc=i["id"]) == 1
    ]
    count = len(result)
    if not count:
        success = 1
    ...
    if count !=0:
        ...
        page_no = int(page_no) if int(page_no)<= total_pages else total_pages
        result = [result[i:i+10] for i in range(0, len(result), 10)]
        success = 1
        msg = "Records Found"
        data = result[int(page_no)-1]
        total_count = count
    frappe.response["success"] = success
    frappe.response["message"] = msg
...
def get_transaction_list(user_id,filters=None,supplier_id=None,from_date=None,to_date=None,page_no=1):
    ...
    if supplier_id!=None and supplier_id!="" and supplier_id!="null":
        values["supplier_id"] = "%%%s%%"%supplier_id
        conditions += " or sp.supplier like %(supplier_id)s" if conditions!="" else "sp.supplier like %(supplier_id)s"
    ...
    result = [
        i for i in all if frappe.has_permission("S4S Purchase", user=user_id, doc=i["transaction_id"]) == 1
    ]

    for i in result:
        purchase_doc = frappe.get_doc("S4S Purchase", i["transaction_id"])
        purchase_receipt_details = purchase_doc.purchase_receipt_details

        if purchase_receipt_details:
            i["product_name"] = purchase_receipt_details[0].item_code
            i["rate"] = purchase_receipt_details[0].rate
        else:
            i["product_name"] = ""
            i["rate"] = ""
    ...
    if count:
        ...
        result = [result[i:i + 10] for i in range(0, len(result), 10)]
        success, msg, data, total_count = 1, "Records Found", result[page_no - 1], count
...
def get_inquiry_list(user_id,status=None,filters=None,farmer_id=None,from_date=None,to_date=None,page_no=1):
    ...
    if farmer_id!=None and farmer_id!="" and farmer_id!="null":
        values["farmer_id"] = "%%%s%%"%farmer_id
        conditions += "or fq.supplier like %(farmer_id)s" if conditions!="" else "fq.supplier like %(farmer_id)s"
    if (from_date!=None and from_date!="" and from_date!="null") and (to_date!=None and to_date!="" and to_date!="null"):
        values["from_date"]=from_date
        values["to_date"]=to_date
    ...
    result = [
        i for i in all if frappe.has_permission("S4S Farmer Query", user=user_id, doc=i["inquiry_id"]) == 1
    ]
    count = len(result)
    if not count:
        success = 1
...
@frappe.whitelist(methods=["GET","POST","PUT"])
def add_farmer(user_id,name,mobile_no,taluka=None, village=None, district=None, state=None , gender=None,cc_name=None,vlcc_name=None):
    success = 0
    msg = "Failed"
    farmerId = ""
    mobNumber = mobile_no
    ...
    if name and mobNumber and exists_check!=1:
        ...
        if user_id and frappe.db.exists("User Permission",{"user":user_id,"allow":"Company"}):
            up = frappe.get_doc("User Permission",{"user":user_id,"allow":"Company"})
            doc.company = up.for_value
        doc.save()
        if doc:
            success = 1
...
def get_transaction_details(transaction_id):
    ...
    if frappe.db.exists("S4S Purchase",transactionId):
        ...
        doc = frappe.get_all("S4S Purchase",filters={"name":transactionId},fields=["name", "supplier", "supplier_name", "transaction_date", "set_warehouse", "delivery_location", "total_weight", "kadta_weight", "weight_after_kadta", "total_amount", "s4s_farmer_query", "kadta", "supplier_deductions", "grand_total", "vlcc_name", "cc_name", "village", "taluka", "district", "approved_status", "vehicle_no", "driver_name", "assigned_vehicle", "total_deduction_amount", "state", "timestamp", "company", "abbr", "s4s_utr_no", "s4s_payment_no", "fpo_utr_no", "fpo_payment_no", "payment_status"])[0]
        success=1
        message="Found"
...
def get_inquiry_details(inquiry_id):
    ...
    if frappe.db.exists("S4S Farmer Query",inquiryId):
        ...
        doc = frappe.get_all("S4S Farmer Query",filters={'name':inquiryId},fields=["name", "supplier", "supplier_name", "item_name", "variety", "moisture", "rate", "product_photo", "comment", "date", "delivery_location", "sample_given", "item_code", "mobile_no", "vlcc_name", "cc_name", "village", "taluka", "district", "s4s_purchase", "tentative_qty", "pickup_request", "vehicle_no", "driver_name", "assigned_vehicle", "state", "status", "company", "abbr"])
        success=1
        message="Found"
...
@frappe.whitelist(methods=["GET","POST","PUT"])
def add_inquiry(user_id,params):
    success = 0
    msg = "Failed"
    # try:
    if type(params) != str:
...
def get_transaction_form_data(user_id):
    success= 0
    msg = "Record Not Found"
    items_data = []
    deduction_data =[]
    ...
    if frappe.db.exists("User Permission",{"user":user_id,"allow":"Company"}):
        up = frappe.get_doc("User Permission",{"user":user_id,"allow":"Company"})
        company = up.for_value
        if company and frappe.db.exists("Purchase Taxes and Charges Template",{"title":"Supplier Deduction","company":company}):
            taxes_doc = frappe.get_doc("Purchase Taxes and Charges Template",{"title":"Supplier Deduction","company":company})
            if taxes_doc.taxes:
...
def get_delivery_challan_list(user_id,stock_entry_id=None,transaction_id=None,page_po=1):
    ...
    if transaction_id!=None:
        values["transaction_id"] = "%%%s%%"%transaction_id
        conditions += "or se.s4s_purchase like %(transaction_id)s" if conditions!="" else " se.s4s_purchase like %(transaction_id)s"
    all = frappe.db.sql("""
        SELECT
    ...
    result = [
        i for i in all if frappe.has_permission("Stock Entry", user=user_id, doc=i["stock_entry_id"]) == 1
    ]
    count = len(result)
    if not count:
        success = 1
    ...
    if count !=0:
        total_pages = math.ceil(int(count)/10)
        page_po = int(page_po) if int(page_po)<= total_pages else total_pages
        result = [result[i:i+10] for i in range(0, len(result), 10)]
        success = 1
        msg = "Records Found"
        data = result[int(page_po)-1]
        total_count = count
    frappe.response["success"] = success
    frappe.response["message"] = msg
    frappe.response["data"] = data
...
def get_purchase_receipt_list(user_id,filters=None,transaction_id=None,page_no=1):
    ...
    if transaction_id!=None:
        values["transaction_id"] = "%%%s%%"%transaction_id
        conditions += "or pr.s4s_purchase like %(transaction_id)s" if conditions!="" else " pr.s4s_purchase like %(transaction_id)s"
    all = frappe.db.sql("""
        SELECT
        ...
        ORDER BY
            pr.creation DESC
        """.format("AND ("+conditions+" )" if conditions!="" else conditions 
        ),
        values,
        as_dict=1
    )
    result = [
        i for i in all if frappe.has_permission("Purchase Receipt", user=user_id, doc=i["purchase_receipt_id"]) == 1
    ]
    count = len(result)
    if not count:
        success = 1
...
def get_purchase_invoice_list(user_id,filters=None,transaction_id=None,page_no=1):
    ...
    if transaction_id!=None:
        values["transaction_id"] = "%%%s%%"%transaction_id
        conditions += "or pi.s4s_purchase like %(transaction_id)s" if conditions!="" else " pi.s4s_purchase like %(transaction_id)s"
    all = frappe.db.sql("""
        SELECT
    ...
    result = [
        i for i in all if frappe.has_permission("Purchase Invoice", user=user_id, doc=i["purchase_invoice_id"]) == 1
    ]
    count = len(result)
    if not count:
        success = 1

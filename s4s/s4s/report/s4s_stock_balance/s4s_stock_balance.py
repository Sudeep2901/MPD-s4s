...
def execute(filters=None):
	...
	for i in data:
		if "warehouse" in i:
			print(i)
			if frappe.db.exists("Warehouse",i["warehouse"]):
				warehouse_doc = frappe.get_doc("Warehouse",i["warehouse"])
				i["warehouse_type"]=warehouse_doc.warehouse_type
				i["warehouse_subtype"]=warehouse_doc.warehouse_subtype
				i["dcm_link"]=warehouse_doc.dcm_link
	return columns, data
...
def get_columns(filters):
	...
	columns = [
		...
		{
			"label": _("Warehouse Sub Type"),
			"fieldname": "warehouse_subtype",
			"fieldtype": "Link",
			"options": "S4S Warehouse Sub Type",
			"width": 100,
		},
		{
			"label": _("DCM Link"),

...
frappe.provide("s4s.fpo");
frappe.provide("erpnext.accounts.dimensions");
{% include 'erpnext/public/js/controllers/buying.js' %};

frappe.ui.form.on("FPO Purchase Order", {
	setup: function(frm) {
		...
		if(frm.doc.company){
			frappe.call({
				...
				callback:function(r){
					if (r.message){
						let data = r.message
						frm.set_value("shipping_address",data.name)
						frm.set_value("billing_address",data.name)
					}
				}
			})
...
frappe.ui.form.on("FPO Purchase Order Item", {
...
s4s.fpo.FPOPurchaseOrderController = erpnext.buying.BuyingController.extend({
	setup: function() {
...
$.extend(cur_frm.cscript, new s4s.fpo.FPOPurchaseOrderController({frm: cur_frm}));
...
frappe.ui.form.on("FPO Purchase Order", "is_subcontracted", function(frm) {

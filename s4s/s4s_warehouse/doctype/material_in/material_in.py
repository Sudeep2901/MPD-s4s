...
class MaterialIn(Document):
    ...
    def make_stock_entry(self):
        ref_stock_entry = frappe.db.get_value(
            "Stock Entry",
            {
                "material_transfer_wh_to_wh": self.material_transfer_wh_to_wh,
                "docstatus": 1,
            },
            ["name"],
        )
        ...
        rej_entry.material_in = self.name
    ...
    def btn_hide_condition(self):
        ...
        if frappe.session.user == "Administrator":
            ...
            if ("S4S SFG Quality User" in roles) or ("System Manager" in roles):
                msg = "Success"
...
def create_qc_checklist(source_name, target_doc=None):
    doc = frappe.get_doc("Material In", source_name)
    qc = frappe.new_doc("ME SFG QC Check List")
    qc.material_in = doc.name

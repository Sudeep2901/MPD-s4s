...
class MESFGTransfertoWIP(Document):
	...
	def create_stock_entry(self):
		if not frappe.db.get_value("Stock Entry",{'me_sfg_transfer_to_wip':self.name},['name']):
			...
			stock_entry.me_sfg_transfer_to_wip = self.name
	...
	def btn_cond_check(self):
		lst = frappe.db.get_list("ME SFG Transfer WIP to FG",{'me_sfg_transfer_to_wip':self.name,'docstatus':['!=',2]})
		if len(lst) == 0:
			return "Yes"
		else:
			return "No"
...
def create_fg_entry(source_name,target_doc=None):
	source = frappe.get_doc("ME SFG Transfer to WIP",source_name)
	target = frappe.new_doc("ME SFG Transfer WIP to FG")
	...
	target.me_sfg_transfer_to_wip = source.name

...
class MESFGProcessingData(Document):
	def get_feed(self):
		pass
	...
	def on_submit(self):
		if not frappe.db.get_value("Stock Entry",{'me_sfg_processing_data':self.name},['name']):
			st_en = frappe.new_doc("Stock Entry")
			st_en.stock_entry_type = "Manufacture"
			st_en.s4ssupplier = self.me_code
			st_en.me_name = self.me_name
			st_en.from_warehouse = self.from_warehouse
			st_en.to_warehouse = self.to_warehouse
			for row in self.items:
				st_en.append("items",{
					...
					"retain_sample":row.retain_sample,
			...
			st_en.me_sfg_processing_data = self.name
			st_en.save(ignore_permissions=True)
			st_en.submit()
	...
	def get_items(self):
		...
		# 	frappe.throw(("Posting date and posting time is mandatory"))

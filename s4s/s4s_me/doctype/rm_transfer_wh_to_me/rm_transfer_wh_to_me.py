...
class RMTransferWHToME(Document):
	...
	def make_stock_entry(self):
		if not frappe.db.get_value("Stock Entry",{'rm_transfer_wh_to_me':self.name},["name"]):
			...
			doc.rm_transfer_wh_to_me = self.name 
		...
		if self.me_code:
			if frappe.db.get_value("ME Onboarding",{'supplier':self.me_code},['name']):
				...
				for row in doc.me_warehouse_details:
					if row.stock_entry == "Material Transfer" and row.add_to_transit == 0:

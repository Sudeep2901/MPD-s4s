...
frappe.ui.form.on('Factory RM Transfer to MFG', {
	setup: function(frm) {
				...
				if(frm.doc.docstatus == 1 && r.message == "Yes"){
					frm.add_custom_button(__('Create FG Entry'), function(){
						frappe.model.open_mapped_doc({
							method: "s4s.s4s_factory.doctype.factory_rm_transfer_to_mfg.factory_rm_transfer_to_mfg.create_fg_entry",
							frm:cur_frm
						})
	...
	onload:function(frm){
		if(frm.doc.rm_item_details.length != 0){
			for(let index in frm.doc.rm_item_details){
				frm.doc.rm_item_details[index].wip_warehouse = frm.doc.wip_warehouse
			}
		}
	},
